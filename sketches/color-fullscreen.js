  ...
  const s02_rotationControlPoints = new Array(s02_maxDepth)
    ...
    .map((_, depth) => ({
      position: [
        0,
        Math.floor(
          ((depth == s02_maxDepth - 1 ? 0 : depth) / (s02_maxDepth - 1)) * 127
        ),
        0,
        0,
      ],
    ...
    new Choreography(
      ...
      function () {
        ...
        s02_circlesDivided = s02_circlesDivided.map((oldGroup, depthIndex) => {
          ...
          if (groupDepth > depthRenderLimit && groupDepth < s02_maxDepth)
            return false;
          ...
          newGroup.children.forEach((path, index) => {
            path.scaling =
              groupDepth < s02_maxDepth ? scale[groupDepth - 1] : 4;
            // path.strokeWidth =
