...
with (paper) {
  ...
  for (let i = 0; i < 45; i++) {
    ...
    const rotations = [s01_rotationArray[0][0]];
    // 2x1
    squishes.push(s01_SquishArray[1][Math.round(gridX / 9 + 1) - 1]);
    rotations.push(s01_rotationArray[1][Math.round(gridX / 9 + 1) - 1]);
    // 3x2
    squishes.push(
      s01_SquishArray[2][
    ...
    rotations.push(
      s01_rotationArray[4][Math.round(gridX + 1) * Math.round(gridY + 1) - 1]
    );
    ...
    const attributeConfigs = [
      ...
      },
      {
        ...
        // dynamicContants: [{}, { f: 2.25, z: 1, r: 0 }],
        controlPoints: rotations.reduce(
          (acc, rotationArray, rotationArrayIndex) =>
            acc.concat(
              rotationArray.map((rotation, rotationIndex) => ({
                position: [
                  0,
                  Math.floor((rotationArrayIndex / steps) * 127),
                  Math.floor((rotationIndex / steps) * 127),
                  0,
                ],
                value: rotation,
              }))
    ...
    scene01.push(
      new Choreography(
        ...
        function () {
          ...
          rect.rotation = 90 * this.rotationScale * (1 - this.scene);
          rect.position = [
        ...
        scene02.push(
          new Choreography(
            ...
            function () {
              ...
              skewAmount *= (skew < 0.5 / 127 ? 0 : skew) * maxRadius;
              skewAmount =
